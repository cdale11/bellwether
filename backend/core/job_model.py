...
class JobModel:
 ...
 def current_wage(self,state,jid):
  ...
  # Modest progression: +B1 after 5 shifts, another after 12; capped.
  ...
  for jid,j in JOBS.items():
   ...
   if not emp:
    ...
    if EVENT_MODEL.job_available(state,jid) and loc==j["location"] and dow in j["days"] and j["start"]<=minute<j["end"] and emp.get("last_shift_day")!=state.get("day"):
     wage=self.current_wage(state,jid); out.append((f"job:work:{jid}",f"Work Shift: {j['name']} ({j['shift_minutes']//60}h, B{wage})"))
 ...
 def work(self,state,jid):
  ...
  return True,f"You spend the shift {j['task']}. At the end of it, you are paid B{wage}.",j["shift_minutes"],wage

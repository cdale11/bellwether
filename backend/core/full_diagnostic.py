...
class FullDiagnosticRunner:
 ...
 def _run(self):
  ...
  try:
   ...
   # Seven-day genuine LLM-player simulation. Every selected action is chosen by the model
   # from the same legal action surface used by the UI; authored story and ending actions are excluded.
   action_count=0; player_audit=[]; start_day=g.state.get("day",1)
   for day_i in range(1,8):
    for turn in range(8):
     call_no=7+action_count
     self._set(55+int(25*((day_i-1)*8+turn)/56),f"AI player · day {day_i}/7 · action {turn+1}/8",call_no)
     action,raw,dt=choose_action(g,"exhaustive but natural seven-day playtest: vary ordinary life, work, social contact, hobbies, gardening and exploration")
     if not action:
      warnings.append(f"AI player found no legal non-story action on diagnostic day {day_i}");break
     before=(g.state.get("day"),g.state.get("minute"),g.state.get("location")); result=g.perform(action["id"]);action_count+=1
     player_audit.append((day_i,action.get("label",action["id"]),dt,before,(g.state.get("day"),g.state.get("minute"),g.state.get("location"))))
     with self.lock:
      self.status.setdefault("feed",[]).append(f"Day {day_i} · {action.get('label',action['id'])} · AI {dt:.1f}s")
      self.status["feed"]=self.status["feed"][-20:]
    # Preserve a week-long test horizon even if chosen actions consumed little time.
    target=start_day+day_i
    if g.state.get("day",1)<target:g.advance((target-g.state.get("day",1))*1440)
   ...
   lines=["BELLWETHER FULL AI STRESS DIAGNOSTIC REPORT","Version: 1.0.10","Scope: real LLM subsystem stress + isolated 7-day LLM player simulation through legal game actions",f"Overall: {score}% ({passed}/{total} checks passed)",f"Elapsed wall time: {elapsed/60:.1f} minutes",f"Actions exercised: {action_count}",f"Real specialist LLM calls exercised: {len(ai_audit)}",f"LLM player decisions exercised: {action_count}","","FAILED CHECKS:"]

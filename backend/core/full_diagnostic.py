 ...
 def _run(self):
  ...
  try:
   ...
   while g.state.get('day',1)<start_day+7 and action_count<96:
    ...
    player_audit.append((dayn,target or 'variety',label,dt,outcome,'changed' if changed else 'NO_EFFECT',before[:3],after[:3]));decision_log.open('a',encoding='utf-8').write(json.dumps({'day':dayn,'target':target,'action':aid,'label':label,'latency_s':round(dt,2),'outcome':outcome,'changed':changed,'goal_progress':bool(meta.get('goal_progress')),'target_stall':target_fail.get(target,0),'before':before,'after':after})+'\n');self._set(self.status['progress'],self.status['phase'],7+account['attempts'],f'Day {dayn} · {target or "variety"} · {label} · {outcome} {dt:.1f}s'+(' · NO EFFECT' if not changed else ''))
   ...
   call_log.write_text('\n'.join(json.dumps(x,default=str) for x in provider.debug_traces),encoding='utf-8');snapshot_log.write_text(json.dumps(g.state,default=str),encoding='utf-8');failures=[x for x in checks if not x[1]];elapsed=(time.time()-started)/60;integrity=round(100*(len(checks)-len(failures))/max(1,len(checks)));lines=['BELLWETHER FULL AI PLAYTEST','Version: 1.2.0','Scope: real LLM subsystem stress + goal-directed isolated play + life-simulation expansion + economy/village-change + controlled procedural/horror certification',f'Overall integrity: {integrity}%',f'Elapsed wall time: {elapsed:.1f} minutes',f'LLM-selected actions exercised: {action_count}',f'Deterministic world minutes after AI budget: {completion_minutes}',f'Real specialist LLM calls exercised: {len(ai_audit)}',f'AI player decision attempts: {account["attempts"]}','','DECISION ACCOUNTING']+[f'- {k}: {v}' for k,v in account.items()]+['','FAILED CHECKS:']+([f'- {n}: {d}' for n,ok,d in failures] or ['- None'])+['','WARNINGS:']+([f'- {x}' for x in warnings] or ['- None'])+['','REAL AI CALL AUDIT:']+[f'- {"PASS" if ok else "FAIL"} | {n} | {dt:.1f}s' for n,dt,ok in ai_audit]+['','AI PLAYER ACTION TRACE:']+[f'- Day {d} | target {tgt} | {label} | {outcome} {dt:.1f}s | {effect} | {before} -> {after}' for d,tgt,label,dt,outcome,effect,before,after in player_audit]+['','CHECKS:']+[f'- {"PASS" if ok else "FAIL"} | {n} | {d}' for n,ok,d in checks]+['','QUALITY DIMENSIONS:',f'- Structural correctness: {round(100*sum(1 for n,ok,d in checks if ok)/max(1,len(checks)))}%',f'- Feature coverage: {round(100*sum(1 for n,ok,d in checks if n.startswith("coverage_") and ok)/max(1,sum(1 for n,ok,d in checks if n.startswith("coverage_"))))}%',f'- AI decision success: {round(100*account["llm_success"]/max(1,account["attempts"]))}%',f'- Fallback rate: {round(100*account["fallbacks"]/max(1,account["attempts"]))}%','','STATE SUMMARY:',f'- Final simulated clock: Day {g.state.get("day")} at {g.time_label()}',f'- World ticks: {wr.get("ticks",0)}',f'- Procedural arcs: active={len(arcs.get("active",[]))}, history={len(arcs.get("history",[]))}, proposals={arcs.get("proposal_count",0)}',f'- Economy ledger entries: {len(econ.get("ledger",[]))}',f'- World events retained: {len(g.state.get("world_events",[]))}',f'- Diagnostic audit directory: {audit_dir}','','LIVE CHECKPOINT FILES:','- diagnostics/latest_live_diagnostic.json','- diagnostics/latest_live_diagnostic.txt']
   report='\n'.join(lines)

...
class FullDiagnosticRunner:
 ...
 def _live_text(self,s):
  return "BELLWETHER LIVE DIAGNOSTIC CHECKPOINT\nVersion: %s\nProgress: %s%%\nPhase: %s\n\nDECISION ACCOUNTING\n%s\n\nRECENT TRACE\n%s\n"%((ROOT/'VERSION').read_text().strip(),s.get('progress',0),s.get('phase'),json.dumps(s.get('decision_accounting',{}),indent=2),'\n'.join('- '+x for x in s.get('feed',[])))
 def start(self):
  with self.lock:

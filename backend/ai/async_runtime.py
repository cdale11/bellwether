    ...
    def status(self):
        with self._lock:
            ...
            return {
                ...
                "oldest_wait_s": round(max([now-v["submitted_at"] for v in self._jobs.values() if v.get("state")=="queued"] or [0.0]),2),
                "inference_accounting": {
                    "requested_decisions": self._metrics.get("job_queued",0)+self._metrics.get("job_merged_before_inference",0)+self._metrics.get("job_request_deferred_running",0),
                    "merged_before_inference": self._metrics.get("job_merged_before_inference",0),
                    "deferred_while_running": self._metrics.get("job_request_deferred_running",0),
                    "calls_started": self._metrics.get("job_started",0),
                    "calls_completed": self._metrics.get("job_finished",0),
                    "calls_failed": self._metrics.get("job_failed",0),
                    "results_applied": self._metrics.get("job_result_applied",0),
                    "results_cancelled": self._metrics.get("job_result_cancelled",0),
                    "completed_inference_wasted": 0},

...
def main():
    ...
    ok("complete core cast graph",len(SOCIAL_MODEL.edges)==len({x for edge in SOCIAL_MODEL.edges for x in edge.split("|")})*(len({x for edge in SOCIAL_MODEL.edges for x in edge.split("|")})-1)//2)

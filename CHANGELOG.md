## 3.20.0
- Unified evidence-based morning report with severity ranking, root-cause grouping, reproduction traces, and cross-agent/archetype comparison.

## v3.19.0
- Added sustained synthetic player archetypes for adaptation testing.
- Added Town Mind/Chorus theory and strategy snapshots.
- Added cross-archetype strategic-signature collision and theory-inactivity diagnostics.
- Extended the existing interrupt-safe overnight report with adaptation evidence.
- Preserved memory, Story Critic, Village Observer, coverage, and JSONL diagnostics.

## v3.18.0
- Added Village Observer sampling to the autonomous QA laboratory.
- Added soak analysis for time stalls, runaway world-event/social-state growth, and rapid serialized-state growth.
- Added observer evidence and findings to the existing copyable overnight report.
- Preserved Human Player, Completionist, Chaos Player, Story Critic, memory instrumentation, backpressure, and interrupt-safe report artifacts.

## v3.17.0
- Added periodic LLM Story Critic reviews to autonomous QA.
- Added structured player-irritation findings to live/final diagnostic reports.
- Preserved deterministic contract verification and v3.16 Test Director roles.
- Preserved interrupt-safe JSONL checkpoints, report viewer, copy and export workflow.

## v3.16.0
- Added autonomous QA Test Director roles: Human Player, Completionist, Chaos Player.
- Preserved interrupt-safe JSONL/live/final diagnostic report pipeline.
- Added per-role action and finding summaries.
- Retained memory backpressure, fallback telemetry, mismatch/repetition detection, and season diagnostics.
...
## 3.1.0
- Reworked player information architecture around a Life Book and intent-oriented action categories.
- Added interpreted Player & Household status information.
- Reduced permanent main-screen duplication.
- Improved Journal, Inventory, Relationships, and Home information hierarchy.
- Added human-readable Developer Overview and Adversary Inspector.
- Rejected leaked SOCIAL metadata boilerplate in foreground dialogue parsing.

## 3.2.0
- Routed action consequences and narration into the scene-image transcript.
- Added cottage-scale animal simulation and bounded asynchronous LLM intention selection.
- Added coop, chickens, ducks, later goat shelter/goat path, feed, care, trust and produce collection.
- Added Home > Animals management surface.

## 3.2.1
- Restored narration to the dedicated narrator strip so action prose does not compete with NPC/player dialogue over the scene image.
- Kept NPC, player, and Bellwether speech in the scene transcript.
- Preserved explicit free-form conversation exchange priority while independently surfacing narration generated by the same action response.

## 3.3.0
- Added prepared-stock production chains to player enterprises.

...
function panelHtml(type){const s=currentGameData.state;if(type==='menu')return `<h3>Game</h3><p>Saving is server-side. Reloading the browser reconnects to the current running game; it does not start over.</p><div class="panel-actions"><button onclick="saveGame()">Save Game</button><button onclick="loadGame()">Load Save</button><button onclick="openPanel('history')">History</button><button class="danger-button" onclick="resetFreshGame()">Reset to Fresh Game</button></div><p><small>Reset starts from the beginning without recurrence carry-over and asks for confirmation first.</small></p>`;if(type==='history')return (s.history||[]).map(l=>`<p class="history-line"><span class="speaker">${escapeHtml(l.speaker)}</span><br>${escapeHtml(l.text)}</p>`).join('')||'<p>No history yet.</p>';
...
const saveGame=()=>lockedUtilityRequest('/api/save'),loadGame=()=>lockedUtilityRequest('/api/load');async function resetFreshGame(){if(!confirm('Start a completely fresh game? Unsaved progress and recurrence carry-over will be lost.'))return;closePanel();await lockedUtilityRequest('/api/reset-fresh');}

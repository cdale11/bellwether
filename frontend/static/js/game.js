...
const quick=actions.filter(a=>a.kind==='choice'||a.kind==='talk'||a.kind==='free_talk'||/water|treat|shelter|begin another run/i.test(a.label)).slice(0,4);const quickIds=new Set(quick.map(a=>a.id));actions.filter(a=>!quickIds.has(a.id)).forEach(a=>groups[categoryFor(a)].push(a));

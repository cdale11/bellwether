...
function render(data){currentGameData=data;const s=data.state,w=s.weather||{};$('location').textContent=data.location.name;$('scene-title').textContent=data.location.name;$('scene-title-overlay').textContent=data.location.name;const season=s.season?.label||'Season unknown';$('status').textContent=`Day ${s.day} · ${s.time} · ${season} · ${w.label} · ${w.temperature_c}°C · £${s.money}`;document.body.dataset.weather=w.state||'clear';document.body.dataset.daypart=s.daypart||'';applyVisualState(data);applyInterfaceHorror(data);
const effect=data.world_context?.weather_effect||'';$('weather-impact').textContent=effect;$('weather-impact').style.display=effect?'block':'none';const placeText=data.location?.description||'Nothing unusual demands your attention.';$('environment-line').textContent=effect||placeText;$('environment-line-overlay').textContent=effect||placeText;
...
const pace=data.simulation_pacing||{};const banner=$('pacing-banner');if(banner){const visible=['high','critical'].includes(pace.band);banner.classList.toggle('hidden',!visible);$('pacing-title').textContent=pace.band==='critical'?'The Village Turns':'Bellwether is catching up';$('pacing-message').textContent=pace.message||'Recent hours are settling into consequence.';}}
